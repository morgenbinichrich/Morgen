// ============================================================
//  Morgen — src/commands/aiCommand.js
//  Google Gemini AI  (CT 1.8.9 / Rhino safe)
//
//  /mm ai                         — help
//  /mm ai key <key>               — set Gemini API key
//  /mm ai status                  — show config
//  /mm ai region <region>         — set API region hint
//  /mm ai ref1                    — capture held item as reference 1
//  /mm ai ref2                    — capture held item as reference 2
//  /mm ai names <amount> <theme>  — generate N tier names
//  /mm ai mig   <amount> <theme>  — generate .mig file with N items
// ============================================================
...
var ref1 = null; // { name, lore[] }
var ref2 = null;

// ─── Gemini HTTP via Apache HttpClient (bundled in Forge) ─────
...
function heldItemRef() {
    ...
    // Keep & color codes — replace § with & so the AI can copy them verbatim
    return {
...
// ─── Build lore template from reference items ─────────────────
// Returns the lore lines of ref1 (preferred) or ref2 as a quoted
// .mig lore block string, to be injected verbatim into the prompt.
...
function buildNamesPrompt(amount, theme) {
    var ctx = refContext();
    // NOTE: examples use [AMP] as a placeholder so CT does not parse them as color codes.
    // Gemini must output literal ampersand codes like &7, &e etc — NOT the § character.
...
function buildMigPrompt(amount, theme) {
    ...
    // Build a realistic name list example with the right count
...
export function handleAiCommand(args) {
    ...
    // ref1 — no extra args = capture, otherwise show
    if (sub === "ref1") {
        if (rest.length === 0) {
            var r = heldItemRef();
    ...
    // names [amount] <theme...>  — amount is optional, falls back to Settings.aiNameCount
    ...
    // mig [amount] <theme...>  — amount is optional, falls back to Settings.aiItemCount
    if (sub === "mig") {
        var firstIsNum = !isNaN(parseInt(rest[0]));
