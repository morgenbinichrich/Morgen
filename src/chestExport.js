...
// ─── Read all items from the open container ───────────────────
...
function doExport() {
    ...
    if (data.items.length === 0) {
        msg("&cNo items found in the open chest" + ((Settings.chestExportSkipLastRow !== false) ? " &8(last row skipped)" : "") + ".");
        return;
    }
    ...
    msg("&a&l✔ &aExport Complete");
    msg("  &7Items   &f" + data.items.length + " &8item" + (data.items.length !== 1 ? "s" : "") + " saved");
    msg("  &7File    &e" + importPath + "&8.mig");
    var importComp = new TextComponent(ChatLib.addColor("  &7Action  "));
    var btnComp = new TextComponent(ChatLib.addColor("&a&l[ ▶ Import Now ]"));
...
// ─── Render overlay (guiRender = draws ON TOP of the open GUI screen) ────────

var _prevMouseDown = false;  // track left-button state across frames
...
register("guiRender", function() {
    try {
        ...
        // Poll LWJGL for left mouse button state
        var mouseDown = false;
        ...
        // Lock / move icon
        var lockX = btnX - 16, lockY = btnY + 5;
        Renderer.drawRect(btnMoving ? Renderer.color(80,60,0,200) : Renderer.color(20,20,20,140), lockX-2, lockY-3, 14, 14);
        Renderer.drawString(btnMoving ? "&6⚿" : "&8⚿", lockX, lockY, true);

        var label = (btnMoving ? "&6✥ " : "") + "⬤ Export All → .mig";
