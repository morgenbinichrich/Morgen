// ============================================================
//  Morgen — gui/QuickSpawnGUI.js
//  FIX: spawn puts item into hotbar slot (0-8), not inventory
// ============================================================
...
var TIER_COLS=[
    ...
    Renderer.color( 80,130, 80,200),
    Renderer.color( 70, 90,160,200),
    Renderer.color(140, 80,180,200),
    Renderer.color(190,140, 30,200),
    Renderer.color(200, 60, 60,200),
    Renderer.color( 40,180,200,200),
    Renderer.color(220,100, 30,200),
...
gui.registerDraw(function(mx,my){
    ...
    Renderer.drawString("&6&lQuick Spawn  &8\u00bb  &7left-click &8= &fspawn to hotbar  &7right-click &8= &fclear",
        L.x+18,L.y+10,true);
    ...
    for(var i=0;i<9;i++){
        ...
        var bc=isHov?GOLD:(slot?TIER_COLS[i]:Renderer.color(42,45,68,180));
        border1(sx,sy,SW-3,SH-3,bc);
        ...
        if(slot){
            ...
            var lbl=slot.label||"?";
            var l1=lbl.substring(0,8);
            var l2=lbl.length>8?lbl.substring(8,14)+(lbl.length>14?"\u2026":""):"";
            Renderer.drawString("&f"+l1,sx+3,sy+18,true);
            if(l2)Renderer.drawString("&f"+l2,sx+3,sy+28,true);
            ...
            // Hotbar target indicator
            Renderer.drawString("&7\u2192 slot "+(i+1),sx+2,sy+38,true);
            if(isHov){
                var tip=slot.path;
    ...
    Renderer.drawString("&8/mm qs set <slot 1-9> <path>   \u00b7   item spawns into hotbar slot   \u00b7   ESC to close",
        L.x+PAD+2,hy+5,true);
});
...
gui.registerClicked(function(mx,my,b){
    ...
    var targetHotbar=hovered; // slot index 0-8 maps directly to hotbar 0-8
    gui.close();
    // FIX: pass hotbar slot as extra arg so giveCommand places item there
    setTimeout(function(){
        // Switch to that hotbar slot first so the item lands in the right place
        Player.setHeldItemIndex(targetHotbar);
        setTimeout(function(){
            ChatLib.command("mmimport "+s.path,true);
        },60);
    },80);
});
...
export function handleQsCommand(args){
    ...
    if(sub==="help"){
        ...
        msg("  &f/mm qs set <1-9> <path>  &8\u2014 &7assign a .mig path to a slot");
        msg("  &f/mm qs clear [1-9]       &8\u2014 &7clear slot (omit number = clear all)");
        msg("  &f/mm qs list              &8\u2014 &7list all assigned slots");
        return;
    }
    // /mm qs <1-9>  — direct spawn from chat
    ...
    if(sub==="list"){
        ...
        for(var i=0;i<9;i++){var s=slots[i];msg("  &e"+(i+1)+" &8\u00bb "+(s?"&f"+s.path:"&8empty"));}
        return;
    }
    openQuickSpawn();
}
