...
export function getNextEmptySlot(start) {
    ...
    if (!inv) return -1;
    // start is the slot to begin scanning FROM (so batch items don't collide).
    // Clamp start into valid ranges. Prefer main inv (9-35), fallback hotbar (36-44).
    const lo = (start !== undefined && start > 9) ? start : 9;
    // Main inventory pass — from lo upward
    for (let i = lo; i <= 35; i++) {
        try { const s = inv.getStackInSlot(i); if (!s || s.getID() === 0) return i; } catch (_) {}
    }
    // Wrap back from slot 9 up to lo-1 (in case earlier slots are empty)
    for (let i = 9; i < lo; i++) {
        try { const s = inv.getStackInSlot(i); if (!s || s.getID() === 0) return i; } catch (_) {}
    }
    // Fall back to hotbar
